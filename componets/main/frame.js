...
var Frame = React.createClass({
  ...
  render: function() {
    ...
    return (
        ...
        <View style={[full_page, {position: 'absolute', width: deviceWidth/2}]}>
          {this.props.leftView}
        </View>
        <View style={[full_page, {position: 'absolute', width: deviceWidth/2, left: deviceWidth/2}]}>
  ...
  //弹出一个新的页面
  popup(view, options){
    var len = this.state.popups.length-1, fling=false;
    while(len>-1){
      fling = this.refs['animate_'+len].isFling();
      if(fling){
        break;
      }
      len --;
